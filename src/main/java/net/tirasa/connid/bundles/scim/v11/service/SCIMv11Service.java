...
import net.tirasa.connid.bundles.scim.v11.dto.Type;
...
public abstract class SCIMv11Service implements SCIMService<SCIMv11User>  {
    ...
    protected void doCreate(final SCIMv11User user, final WebClient webClient) {
    ...
    protected JsonNode doUpdate(final SCIMv11User user, final WebClient webClient) {
    ...
    private JsonNode buildCustomAttributesNode(final String customAttributesJSON, final SCIMv11User user) {
        ...
        if (StringUtil.isNotBlank(customAttributesJSON) && !user.getSCIMCustomAttributes().isEmpty()) {
            ...
            for (SCIMv11Attribute scimAttribute : user.getSCIMCustomAttributes().keySet()) {
                if (scimAttribute.getType().equals(SCIMAttributeUtils.SCIM_SCHEMA_TYPE_COMPLEX)) {
                    for (SCIMv11Attribute scimSubAttribute : scimAttribute.getSubAttributes()) {
    ...
    protected <T extends SCIMBaseAttribute<T>> void readCustomAttributes(final SCIMv11User user, final JsonNode node) {
            ...
            if (scimSchema != null && !scimSchema.getAttributes().isEmpty()) {
                for (T attribute : scimSchema.getAttributes()) {
                    List<JsonNode> foundWithSchemaAsKey =
                            node.findValues(SCIMv11Attribute.class.cast(attribute).getSchema());
    ...
    protected void readCustomAttributes(final PagedResults<SCIMv11User> resources, final JsonNode node) {
        for (SCIMv11User resource : resources.getResources()) {
